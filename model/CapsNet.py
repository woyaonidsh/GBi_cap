...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, num_conv_units=16, in_channels=256, out_channels=64, kernel_size=9, stride=2):
...
class CapsNet(nn.Module):
    ...
    def forward(self, x):
        out = self.relu(self.conv(x))   # 经过一层CNN
        out = self.primary_caps(out)
        out = self.digit_caps(out)

        # Shape of logits: (batch_size, out_capsules)
        logits = torch.norm(out, dim=-1)
        pred = torch.eye(24).to(device).index_select(dim=0, index=torch.argmax(logits, dim=1))

        # Reconstruction
        reconstruction = self.decoder((out * pred.unsqueeze(2)).contiguous())
...
inputs = torch.randn(16, 1, 36, 36).to(device)
